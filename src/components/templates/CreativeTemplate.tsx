...
const ACCENT = "#e05c3a"; // warm coral

import { ContactLinks, getContactHTML } from ".";

export function CreativeTemplate({ cv, name, email, phone, city, linkedinUrl, websiteUrl }: TemplateProps) {
...
const styles: Record<string, React.CSSProperties> = {
  ...
  name: { fontSize: "26pt", fontWeight: "bold", margin: 0, color: "#1a1a1a" },
  contact: { fontSize: "9pt", color: "#555", marginTop: "5pt" },
  section: { marginBottom: "14pt" },
  sectionTitleWrap: { marginBottom: "6pt" },
  sectionTitle: {
    fontSize: "11pt", fontWeight: "bold", color: "#fff",
    background: ACCENT, display: "inline-block",
    padding: "2pt 10pt", margin: 0, letterSpacing: "0.5pt",
  },
  bodyText: { margin: "0 0 3pt 0", color: "#333" },
  skillRow: { marginBottom: "3pt", color: "#333" },
...
export function toHTML(cv: GeneratedCV, contact: { name: string; email?: string; phone?: string; city?: string; linkedinUrl?: string; websiteUrl?: string }): string {
  ...
  const sectionsHTML = cv.sections.map((s) => {
    ...
    if (s.type === "summary") body = `<p style="margin:0;color:#333">${c?.text ?? ""}</p>`;
      ...
      body = c?.text ? `<p style="margin:0;color:#333">${c.text}</p>` : "";
    ...
    return `<div style="margin-bottom:14pt">
      <div style="margin-bottom:6pt"><h2 style="font-size:11pt;font-weight:bold;color:#fff;background:${ACCENT};display:inline-block;padding:2pt 10pt;margin:0">${s.title}</h2></div>
      ${body}
    </div>`;
  }).join("");
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"/>
    ...
    <div style="background:${ACCENT}12;border-top:4px solid ${ACCENT};padding:18pt 20pt 14pt;margin-bottom:16pt">
      <h1 style="font-size:26pt;font-weight:bold;margin:0;color:#1a1a1a">${contact.name}</h1>
      <div style="font-size:9pt;color:#555;margin-top:5pt">${contactLine}</div>
    </div>
    ${sectionsHTML}
  </body></html>`;
}
